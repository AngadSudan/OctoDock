...
const HudCard = ({ title, subtitle, icon }: any) => (
  ...
  >
    {/* Corner Brackets */}
    {["left-2 top-2", "right-2 top-2 rotate-90", "left-2 bottom-2 -rotate-90", "right-2 bottom-2 rotate-180"].map(
      (pos, i) => (
        <span key={i} className={`absolute h-3 w-3 border-t border-l border-rose-400/60 ${pos}`} />
      )
    )}
    ...
    <div className="flex items-center gap-3">
      ...
      <div>
        <p className="text-[10px] tracking-widest text-rose-300/80 uppercase">Status: Online</p>
        <h4 className="text-slate-100 font-semibold text-lg">{title}</h4>
      </div>
    </div>
    {subtitle && <p className="mt-1 text-xs text-slate-400/80 uppercase">{subtitle}</p>}
  </motion.div>
);

const SectionHeading = ({ id, icon, children }: any) => (
  <h3 id={id} className="group scroll-mt-28 text-xl md:text-2xl font-semibold text-slate-100 mt-10 mb-6 flex items-center gap-3 border-b border-rose-500/10 pb-2">
    ...
    {children}
    <a href={`#${id}`} className="opacity-0 group-hover:opacity-100 transition ml-1 text-rose-400 text-base">#</a>
  </h3>
);
      ...
      <div className="border-b border-white/5 bg-black/20 backdrop-blur sticky top-0 z-20">
        <div className="mx-auto max-w-6xl px-4 h-14 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm tracking-wider text-slate-200">GITHUB OAUTH POLICY</p>
          </div>
          <p className="text-[11px] tracking-widest text-rose-300/80">EFFECTIVE: {new Date(effectiveDate).toDateString()}</p>
        </div>
      </div>
        ...
        <div className="mx-auto max-w-6xl px-4 py-12 md:py-16">
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
            <div className="text-center">
              <h1 id="pp-title" className="text-4xl md:text-6xl font-extrabold text-white" style={{ textShadow: "0 0 18px rgba(244,63,94,0.45)" }}>
                PRIVACY POLICY
              </h1>
              <p className="mt-3 text-lg text-rose-400 tracking-wider">
                <span className="text-slate-400">Section for </span><strong>GitHub OAuth Integration</strong>
              </p>
            </div>

            <div className="mt-10 grid grid-cols-1 sm:grid-cols-3 gap-6">
              <HudCard title="Token Security" subtitle="Hashed & Verified" icon={<Cpu size={18} />} />
              <HudCard title="Scoped Access" subtitle="Minimal Permissions" icon={<Zap size={18} />} />
              <HudCard title="Data Retention" subtitle="Temporary & Minimal" icon={<DatabaseZap size={18} />} />
            </div>
          </motion.div>
        </div>
      </section>
      ...
      <section className="relative pb-20 z-10">
        ...
        <div className="mx-auto max-w-6xl px-4 grid items-start grid-cols-1 gap-12">
          ...
          <article className="prose prose-invert max-w-none min-w-0 z-10 prose-p:leading-relaxed prose-li:marker:text-rose-300 prose-a:text-rose-400 hover:prose-a:text-rose-300">
            ...
            <SectionHeading id="info-we-collect" icon={<Users size={18} />}>1. Information We Collect Through GitHub OAuth</SectionHeading>
            <p>
              When you sign in to <strong>{companyName}</strong> using GitHub OAuth, we receive certain
            ...
            <SectionHeading id="how-we-use" icon={<Github size={18} />}>2. How We Use GitHub OAuth Data</SectionHeading>
            <p>
            ...
            <SectionHeading id="token-storage" icon={<Lock size={18} />}>3. Token Storage and Security</SectionHeading>
            <p>
            ...
            <SectionHeading id="private-repos" icon={<Database size={18} />}>4. Access to Private Repositories</SectionHeading>
            <p>
            ...
            <SectionHeading id="revoking-access" icon={<RefreshCcw size={18} />}>5. Revoking Access</SectionHeading>
            <p>
              ...
              >
                {' '}https://github.com/settings/applications <ExternalLink size={16} className="inline ml-1" />
              </a>
            ...
            <SectionHeading id="data-retention" icon={<Timer size={18} />}>6. Data Retention</SectionHeading>
            <p>
            ...
            <SectionHeading id="protection" icon={<ShieldCheck size={18} />}>7. How We Protect Your Data</SectionHeading>
            <p>
            ...
            <SectionHeading id="sharing" icon={<Users size={18} />}>8. Sharing and Third Parties</SectionHeading>
            <p>
            ...
            <SectionHeading id="rights" icon={<Mail size={18} />}>9. Your Rights and Contact</SectionHeading>
            <p>
              Depending on your jurisdiction, you may have rights to access, correct,
              or delete your personal data. To exercise those rights or for privacy
              inquiries, contact us via email at{" "}
